...
const BTC_DEVICE = testMocks.getSuiteDevice({ state: { staticSessionId: BTC_DEVICE_SSID } });

const ETH_DEVICE_SSID: `${string}@${string}:${number}` = '1stTestnetAddress@device_id:0';
const ETH_DEVICE = testMocks.getSuiteDevice({ state: { staticSessionId: ETH_DEVICE_SSID } });
